...
// 2006 added
void CKServerGeneralPage::ConnectRemote() 
	{
	// Create a wait cursor object.  This will cause the wait cursor, 
	// usually an hourglass, to be displayed.  When this object goes
	// out of scope, its destructor will restore the previous cursor
...
//	hParent = m_pServerList->InsertItem (lpszDataAccessServers20, ILI_CATAGORY, ILI_CATAGORY);
//	DisplayComponentCatList (hParent, CATID_OPCDAServer20);
	hParent = m_pServerList->GetFirstVisibleItem();
	hDAserver20 = m_pServerList->GetNextVisibleItem(hParent);
	DisplayComponentCatList1 (hDAserver20, CATID_OPCDAServer20);
...
// 2006 added
void CKServerGeneralPage::DisplayComponentCatList1 (HTREEITEM hParent, CATID catid)
	{
	HRESULT hr;
...
//////////////////////////////////////
			MULTI_QI m_arrMultiQI [1];	// array of interfaces we will query for each server
			...
			m_arrMultiQI [MQI_IOPCSERVER].pIID		= &IID_IOPCServerList;	// 2006
...
/////////////
		...
		if (SUCCEEDED (hr))
			{
			CATID arrcatid [1];
			arrcatid [0] = catid;	// catid = IID_CATID_OPCDAServer20

			IEnumCLSID *pEnum = NULL;

			// Enumerate registered components based on clsid:
			pCat = (IOPCServerList *) m_arrMultiQI [0].pItf;
			hr = pCat->EnumClassesOfCategories (
//				sizeof (arrcatid) / sizeof (CATID),	// number of catids in the array that follows
				1,									// number of catids in the array that follows
				arrcatid,							// catid array
				0, 
				NULL,
				&pEnum);							// clsid enumerator for registered components under this category
			...
			if (SUCCEEDED (hr))
				...
				while ((hr = pEnum->Next (1, &guid, &fetched)) == S_OK)
					...
					hr = pCat->GetClassDetails (guid, &str1, &str2); 
					nCount = wcstombs( myStr, str1, 255 );
...
//						_wcstombsz (szProgID, wszProgID, sizeof (szProgID) / sizeof (TCHAR));
						m_pServerList->InsertItem (myStr, ILI_COMPONENT, ILI_COMPONENT, hParent);
#endif
						// It is up to us to free the Prog ID string memory:
						CoTaskMemFree (str1);
						}
...
void CKServerGeneralPage::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
